...
export default function Projects() {
  return (
    <section id="projects" className="mt-8 md:mt-12">
      ...
      {/* AgentFence — Full-width interactive terminal demo */}
      {deployedProjects.filter(p => p.component === 'agentFence').map((project, index) => (
        <motion.div
          key={project.id}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          ...
          className="mb-8 rounded-xl bg-charcoal border border-white/5 overflow-hidden hover:border-primary/30 transition-all duration-500"
        >
          <div className="p-6 md:p-8">
            <div className="flex flex-wrap items-center gap-3 mb-3">
              <span className="inline-block px-3 py-1 text-[10px] font-mono tracking-wider uppercase text-primary/80 bg-primary/10 rounded-full">
                {project.category}
              </span>
              <span className="inline-block px-3 py-1 text-[10px] font-mono tracking-wider uppercase text-emerald-400/80 bg-emerald-400/10 rounded-full animate-pulse">
                ● LIVE DEMO
              </span>
            </div>
            <h3 className="font-poppins font-medium text-xl text-white mb-2">{project.title}</h3>
            <p className="text-gray-400 text-sm leading-relaxed mb-5 font-light max-w-3xl">{project.desc}</p>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-white/5">
              {project.live && (
                <a href={project.live} target="_blank" rel="noreferrer"
                  ...
                  <FaExternalLinkAlt className="w-3 h-3" /> Live Site
                </a>
              )}
      ...
      {/* Other deployed projects (without AgentFence) */}
