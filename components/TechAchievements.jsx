...
const achievements = [
  ...
  {
    id: 3,
    title: '8+ Production Apps & Tools Deployed',
    description: 'Designed and deployed full-stack web applications and developer tools — including AgentFence (MCP security guardrail), WorkSphere OS, CloudSecure, and PricePulse.',
  ...
  {
    id: 4,
    title: 'AI-Powered Security & Guardrails',
    description: 'Engineered AgentFence (MCP security guardrail for AI coding agents), network intrusion detection, and API security monitors to intercept leaks and malicious actions.',
...
export default function TechAchievements() {
  ...
  return (
    <section id="achievements" className="mt-8 md:mt-12">
      <SectionHeading label="Track Record" title="Experience & Impact" />
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {achievements.map((achievement, index) => (
          <motion.div
            ...
            className="group relative p-6 md:p-8 rounded-2xl bg-white/[0.03] border border-white/5 backdrop-blur-xl hover:border-primary/30 transition-all duration-300 overflow-hidden shine-sweep"
          >
            {/* Gradient background on hover */}
            <div className={`absolute inset-0 bg-gradient-to-br ${achievement.color} opacity-0 group-hover:opacity-[0.06] transition-opacity duration-300 -z-10`} />

            {/* Glow effect */}
            <div className={`absolute -inset-1 bg-gradient-to-r ${achievement.color} rounded-2xl opacity-0 group-hover:opacity-15 blur-xl transition-opacity duration-300 -z-20`} />

            {/* Metric Badge */}
            <div className="flex items-center justify-between mb-4">
              <div className="text-primary group-hover:drop-shadow-[0_0_10px_rgba(212,175,55,0.3)] transition-all duration-300">
                {achievement.icon}
              </div>
